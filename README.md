...
A file transfer protocol (FPT) can be used to transfer files from one host, to another host machine over a TCP-based network (i.e. internet). Specifically, the raspberry pi can be enabled with an FTP server. This would allow other machines exchanging files with other machines.
